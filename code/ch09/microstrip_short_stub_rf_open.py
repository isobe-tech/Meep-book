...
class ShortStubParams:
    ...
    # Short-circuited stub (extend toward +y; short via to GND at the end)
    l_stub: float = 18.0  # [mm] (initial λg/4 guess)
    x_stub: float = 0.0  # [mm]
    via_r: float = 0.6  # [mm]
    via_z_center: float = -0.5  # coefficient of h (center.z = via_z_center*h)
    ...
    # Stopping condition (does not need to be as strict as a resonator, but should not roughen the band)
...
def make_microstrip_short_stub_geometry(
    *,
    eps_r: float,
    h: float,
    t_metal: float,
    ...
    # Short: connect the stub end to GND via a via (parasitic via inductance matters in practice)
    y_end = 0.5 * w0 + l_stub
    via_h = h + 2 * t_metal + 0.5  # add margin so the via pierces top metal through GND reliably
    via = mp.Cylinder(
        radius=via_r,
        height=via_h,
        center=mp.Vector3(x_stub, y_end, via_z_center * h),
        material=mp.metal,
    )
    geom.append(via)
    return geom, y_end
...
def run_with_short_stub(
    *,
    ...
    geom, y_via = make_microstrip_short_stub_geometry(
        eps_r=p.eps_r,
        h=p.h,
        t_metal=p.t_metal,
        w0=p.w0,
    ...
    sim.reset_meep()
    return p_ref, p_tran, ez_xy, y_via
...
def main() -> None:
    ...
    out_sparams = str(figs_dir / "ch09_short_stub_sparams.png")
    out_ez_xy = str(figs_dir / "ch09_short_stub_ez_xy.png")
    outputs = [out_sparams, out_ez_xy]
    if os.environ.get("CH09_FORCE_REGEN") != "1" and all(os.path.exists(pp) for pp in outputs):
        return
    ...
    f_qw_est = (c0 / L0) / (4.0 * p.l_stub * np.sqrt(eps_eff)) / 1e9
    if mp.am_master():
        print(f"eps_eff≈{eps_eff:.3g}, f_qw_est≈{f_qw_est:.3g} GHz (short→open)")
    ...
    p_ref, p_tran, ez_xy, y_via = run_with_short_stub(
        p=p,
        fcen=fcen,
        df=df,
        df_src=df_src,
    ...
    ax = axes[0]
    ax.plot(f_ghz, s11_db, lw=1.9, label="with short stub (via)")
    ...
    ax.plot(f_ghz, s21_db, lw=1.9, ls="--", label="with short stub (via)")
    ax.set_xlabel("f [GHz]")
    ...
    ax.add_patch(Circle((p.x_stub, y_via), radius=p.via_r, fill=False, lw=1.1, ec="white", alpha=0.9))
    ax.text(
        2.5,
        14.0,
        "short stub (via)",
